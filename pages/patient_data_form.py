...
class PatientDataFormWindow(QtWidgets.QFrame):
    ...
    def ui(self):
        ...
        # 將標題區塊(layer 1)加入到內容區塊(layer 0)的排版
        root_layout.addWidget(title_content)
        # ===== 第一母區塊 - Title =====#

        # ===== 第二母區塊 - basic information =====#
        # 創建 & 設定基本資料背景區塊(layer 1)
        ...
        basic_info_parent_widget.setFixedHeight(
            200
        )  # 調整基本資料背景區塊(layer 1)的大小

        # 創建 & 設定基本資料標題區塊(layer 2)
        ...
        # 創建 & 設定基本資料填寫區塊(layer 2)
        basic_info_input_widget = QtWidgets.QWidget()
        basic_info_input_widget.setStyleSheet("border: 0px;")
        basic_info_input_widget_layout = QtWidgets.QGridLayout(
            basic_info_input_widget
        ...
        gender_group = QtWidgets.QButtonGroup(
            self
        )  # 把RadioButton 加入 ButtonGroup，確保互斥
        ...
        birthday_input.setStyleSheet(
            f"font-size: 16px; font-family: 微軟正黑體; font-weight: bold; background-color: rgb(255, 255, 255); border: 1px solid black; border-radius: 5px"
        )

        # 將基本資料填寫區塊(layer 2)的內容加入到 basic_infos 字典中
        ...
        # 標題區塊(layer 2)加入到基本資料背景區塊(layer 1)的排版
        basic_info_parent_widget_layout.addWidget(label_base_info)

        # basic_info_input_widget(layer 2)加入到基本資料背景區塊(layer 1)的排版
        basic_info_parent_widget_layout.addWidget(basic_info_input_widget)

        # 將基本資料背景區塊(layer 1)加入到內容區塊(layer 0)的排版
        root_layout.addWidget(basic_info_parent_widget)
        # ===== 第二母區塊 - basic information =====#

        # ===== 第三母區塊 - 6QDS ===== #
        # 創建 & 設定6QDS問卷區塊(layer 1)
        Q6DS_parent_widget = QtWidgets.QWidget()
        Q6DS_parent_widget.setStyleSheet(
            "background-color: rgb(214, 234, 248); border: 2px solid black; border-radius: 20px;"
        )
        Q6DS_parent_widget.setFixedHeight(150)  # 調整6QDS問卷區塊(layer 1)的大小
        Q6DS_parent_widget_layout = QtWidgets.QGridLayout(Q6DS_parent_widget)
        Q6DS_parent_widget_layout.setSpacing(10)

        # 創建 & 設定6QDS問卷標題區塊(layer 2)
        Q6DS_label_widget = QtWidgets.QLabel("6QDS")
        Q6DS_label_widget.setStyleSheet(
            f"font-size: 16px; font-family: 微軟正黑體; font-weight: bold; border: 0px"
        )
        Q6DS_label_widget.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)  # 文字置中

        # 創建 & 設定6QDS問卷填寫區塊(layer 2)
        ...
        for i in range(1, 11):
            ...
            # TODO: 要把輸入值用另一個字典儲存
            self.basic_infos[f"q{i}"] = (
                question_input  # 將每個問題的輸入框加入到 basic_infos 字典中
            )

        # 將6QDS問卷標題區塊(layer 2)加入到6QDS問卷區塊(layer 1)的排版
        Q6DS_parent_widget_layout.addWidget(Q6DS_label_widget)

        # 將6QDS問卷填寫區塊(layer 2)加入到6QDS問卷區塊(layer 1)的排版
        Q6DS_parent_widget_layout.addWidget(Q6DS_answer_widget)

        # 將6QDS問卷區塊(layer 1)加入到內容區塊(layer 0)的排版
        root_layout.addWidget(Q6DS_parent_widget)
        # ===== 第三母區塊 - 6QDS ===== #

        # ===== 第四母區塊 - save path ===== #
        # 創建 & 設定儲存路徑標題區塊(layer 1)
        save_path_parent_widget = QtWidgets.QWidget()
        save_path_parent_widget.setStyleSheet(
            "background-color: rgb(214, 234, 248); border: 2px solid black; border-radius: 20px;"
        )
        save_path_parent_widget.setFixedHeight(80)  # 調整儲存路徑區塊(layer 1)的大小
        save_path_parent_widget_layout = QtWidgets.QGridLayout(save_path_parent_widget)

        # 創建 & 設定儲存路徑標籤區塊(layer 2)
        save_path_label = QtWidgets.QLabel("儲存路徑 :")
        save_path_label.setStyleSheet(
            "font-size: 16px; font-family: 微軟正黑體; font-weight: bold; border: 0px;"
        )

        # 創建 & 設定儲存路徑填寫區塊(layer 2)的輸入框
        ...
        # 將儲存路徑填寫區塊(layer 2)的內容加入到 save_path_text 字典中
        self.save_path_text["save_path_input"] = save_path_input

        # 將儲存路徑標籤區塊(layer 2)加入到儲存路徑標題區塊(layer 1)的排版
        save_path_parent_widget_layout.addWidget(save_path_label, 0, 0)

        # 將儲存路徑填寫區塊(layer 2)加入到儲存路徑標題區塊(layer 1)的排版
        save_path_parent_widget_layout.addWidget(save_path_input, 0, 1)

        # 將儲存路徑標題區塊(layer 1)加入到內容區塊(layer 0)的排版
        root_layout.addWidget(save_path_parent_widget)
        # ===== 第四母區塊 - savepath =====#
...
if __name__ == "__main__":
    ...
    info_window = PatientDataFormWindow()
